...
import io.scalac.slack.bots.prassign.{UserIdRepository, PrAssignBot}
...
//    val tagBot = context.actorOf(Props(classOf[TagsBot], new TagsRepository(), bus), "tagBot")
    ...
    val prAssignBot = context.actorOf(Props(classOf[PrAssignBot], new UserIdRepository(), bus), "prBot")
